// Asegúrate de que dungeonCooldowns esté disponible globalmente
if (!global.dungeonCooldowns) {
  global.dungeonCooldowns = {};
}
...
let handler = async (m, { conn, text, command }) => {
  ...
  if (global.dungeonCooldowns[m.sender] && Date.now() - global.dungeonCooldowns[m.sender] < tiempoEspera * 1000) {
    let tiempoRestante = segundosAHMS(Math.ceil((global.dungeonCooldowns[m.sender] + tiempoEspera * 1000 - Date.now()) / 1000));
    m.reply(`🗡️ Ya has explorado las mazmorras recientemente. Espera ⏳ *${tiempoRestante}* antes de intentarlo de nuevo.`);
    return;
  }

  // Actualizar el tiempo de enfriamiento
  global.dungeonCooldowns[m.sender] = Date.now();
  ...
  // Simular un encuentro en las mazmorras
  const monsters = [
