...
const isLinkWhtsAppGroup = /chat\.whatsapp\.com/i 
const isLinkWhtsAppChannel = /whatsapp\.com\/channel/i 
...
handler.before = async function (m, { conn, args, usedPrefix, command, isAdmin, isBotAdmin }) {
  ...
  const isAntiLinkWhtsAppGroup = isLinkWhtsAppGroup.exec(m.text) 
  const isAntiLinkWhtsAppChannel = isLinkWhtsAppChannel.exec(m.text)
  ...
  if (chat.antiWhatsappGroup && isAntiLinkWhtsAppGroup) {
    if (isBotAdmin && bot.restrict) {
      await conn.reply(m.chat, `『✦』Se detecto un enlace de \`Grupo WhatsApp\`.\nSeras eliminado/a: *@${toUser}*`, null, { mentions: [aa] })
      await conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
      await conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
    } else if (!isBotAdmin) {
      return m.reply(`『✦』El bot no es admin, no puedo eliminar personas.`)
  ...
  if (chat.antiWhatsappChannel && isAntiLinkWhtsAppChannel) {
    if (isBotAdmin && bot.restrict) {
      await conn.reply(m.chat, `『✦』Se detecto un enlace de \`Canal WhatsApp\`.\nSeras eliminado/a: *@${toUser}*`, null, { mentions: [aa] })
      await conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
      await conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
    } else if (!isBotAdmin) {
      return m.reply(`『✦』El bot no es admin, no puedo eliminar personas.`)
